...
document.querySelectorAll('.navbar a').forEach(link => {
  link.addEventListener('click', function (e) {
    ...
    // If it's an in-page section (starts with #), do smooth scroll
    if (href.startsWith('#')) {
      e.preventDefault();
    ...
    // If it's a normal page (like contact.html), allow default navigation
  });
});
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  circles.forEach(circle => {
    const value = circle.dataset.percent;
